...
Adafruit_ADS1X15::Adafruit_ADS1X15(TwoWire &w, uint8_t i2cAddr)
  : wire(w), i2c_addr(i2cAddr) {
}
...
*/
/**************************************************************************/
Adafruit_ADS1015::Adafruit_ADS1015(TwoWire &w, uint8_t i2cAddr) : Adafruit_ADS1X15(w, i2cAddr) {
  m_bitShift = 4;
...
*/
/**************************************************************************/
Adafruit_ADS1115::Adafruit_ADS1115(TwoWire &w, uint8_t i2cAddr) : Adafruit_ADS1X15(w, i2cAddr) {
  m_bitShift = 0;
  m_gain = GAIN_TWOTHIRDS; /* +/- 6.144V range (limited to VDD +0.3V max!) */
...
bool Adafruit_ADS1X15::begin() {
  ...
  return (this->wire.endTransmission() == TwoWire::SUCCESS);
}
...
uint16_t Adafruit_ADS1X15::readRegister(uint8_t reg) {
  ...
  this->wire.beginTransmission(this->i2c_addr);
  this->wire.write(buffer, 1);
  this->wire.endTransmission(true);

  this->wire.requestFrom(this->i2c_addr, 2);
  buffer[0] = this->wire.read();
  buffer[1] = this->wire.read();
