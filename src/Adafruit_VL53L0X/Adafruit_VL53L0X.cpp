...
#include "../../inc/Adafruit_VL53L0X.hpp"
#include "../../inc/Adafruit_VL53L0X/vl53l0x_api_core.h"
...
Adafruit_VL53L0X::Adafruit_VL53L0X(TwoWire &wire, uint8_t i2cAddr)
  : wire(wire), I2CAddr(i2cAddr) {
}
...
*/
/**************************************************************************/
boolean Adafruit_VL53L0X::begin(VL53L0X_Sense_config_t vl_config, boolean debug) {
  ...
  // Initialize Comms
  MyDevice.I2cDevAddr = this->I2CAddr; // default
  MyDevice.comms_type = 1;
  MyDevice.comms_speed_khz = 400;
  MyDevice.i2c = &this->wire;

  MyDevice.i2c->begin(); // VL53L0X_i2c_init();
...
boolean Adafruit_VL53L0X::configSensor(VL53L0X_Sense_config_t vl_config) {
  ...
  Status = VL53L0X_SetLimitCheckEnable(
      &MyDevice, VL53L0X_CHECKENABLE_SIGMA_FINAL_RANGE, 1);

  if (Status == VL53L0X_ERROR_NONE) {
    Status = VL53L0X_SetLimitCheckEnable(
        &MyDevice, VL53L0X_CHECKENABLE_SIGNAL_RATE_FINAL_RANGE, 1);
  }
  ...
  case VL53L0X_SENSE_DEFAULT:
    ...
    if (Status == VL53L0X_ERROR_NONE) {
      Status = VL53L0X_SetLimitCheckEnable(
          &MyDevice, VL53L0X_CHECKENABLE_RANGE_IGNORE_THRESHOLD, 1);
    }

    if (Status == VL53L0X_ERROR_NONE) {
      Status = VL53L0X_SetLimitCheckValue(
          &MyDevice, VL53L0X_CHECKENABLE_RANGE_IGNORE_THRESHOLD,
  ...
  case VL53L0X_SENSE_LONG_RANGE:
    ...
    if (Status == VL53L0X_ERROR_NONE) {
      Status = VL53L0X_SetVcselPulsePeriod(
          &MyDevice, VL53L0X_VCSEL_PERIOD_FINAL_RANGE, 14);
    }
    break;
  case VL53L0X_SENSE_HIGH_SPEED:
    ...
    if (Status == VL53L0X_ERROR_NONE) {
      Status = VL53L0X_SetMeasurementTimingBudgetMicroSeconds(&MyDevice, 30000);
    }
    break;
  case VL53L0X_SENSE_HIGH_ACCURACY:
    ...
    if (Status == VL53L0X_ERROR_NONE) {
      Status = VL53L0X_SetLimitCheckEnable(
          &MyDevice, VL53L0X_CHECKENABLE_RANGE_IGNORE_THRESHOLD, 0);
    }
...
VL53L0X_Error Adafruit_VL53L0X::getSingleRangingMeasurement(
  ...
  if (Status == VL53L0X_ERROR_NONE) {
    ...
    if (debug) {
      VL53L0X_GetLimitCheckCurrent(&MyDevice,
...
VL53L0X_Error Adafruit_VL53L0X::setInterruptThresholds(
  ...
  if (Status == VL53L0X_ERROR_NONE) {
    ...
    Status = VL53L0X_SetInterruptThresholds(
        &MyDevice, VL53L0X_DEVICEMODE_CONTINUOUS_RANGING, ThresholdLow,
        ThresholdHigh);
  }
...
Adafruit_VL53L0X::getInterruptThresholds(FixPoint1616_t *pThresholdLow,
  ...
  if (Status == VL53L0X_ERROR_NONE) {
    ...
    Status = VL53L0X_GetInterruptThresholds(
        &MyDevice, VL53L0X_DEVICEMODE_CONTINUOUS_RANGING, pThresholdLow,
        pThresholdHigh);
  }
...
VL53L0X_Error Adafruit_VL53L0X::setGpioConfig(
  ...
  if (Status == VL53L0X_ERROR_NONE) {
    ...
    Status = VL53L0X_SetGpioConfig(&MyDevice, 0, DeviceMode, Functionality,
                                   Polarity);
  }
...
VL53L0X_Error Adafruit_VL53L0X::getGpioConfig(
  ...
  if (Status == VL53L0X_ERROR_NONE) {
    ...
    Status = VL53L0X_GetGpioConfig(&MyDevice, 0, pDeviceMode, pFunctionality,
                                   pPolarity);
  }
...
boolean Adafruit_VL53L0X::waitRangeComplete(void) {
  Status = VL53L0X_measurement_poll_for_completion(&MyDevice);
...
uint16_t Adafruit_VL53L0X::readRangeResult(void) {
  ...
  Status = VL53L0X_GetRangingMeasurementData(&MyDevice, &measure);
  _rangeStatus = measure.RangeStatus;
  if (Status == VL53L0X_ERROR_NONE)
    Status = VL53L0X_ClearInterruptMask(&MyDevice, 0);
...
boolean Adafruit_VL53L0X::startRangeContinuous(uint16_t period_ms) {
   ...
   * Here we fix the mode! */
  // first lets set the device in SINGLE_Ranging mode
  Status = VL53L0X_SetDeviceMode(&MyDevice,
                                 VL53L0X_DEVICEMODE_CONTINUOUS_TIMED_RANGING);

  if (Status == VL53L0X_ERROR_NONE) {
    Status =
        VL53L0X_SetInterMeasurementPeriodMilliSeconds(&MyDevice, period_ms);
  }
...
void Adafruit_VL53L0X::stopRangeContinuous(void) {
  ...
  if (Status == VL53L0X_ERROR_NONE) {
    LoopNb = 0;
    do {
      Status = VL53L0X_GetStopCompletedStatus(&MyDevice, &StopCompleted);
  ...
  if (Status == VL53L0X_ERROR_NONE) {
    Status = VL53L0X_ClearInterruptMask(
        &MyDevice, VL53L0X_REG_SYSTEM_INTERRUPT_GPIO_NEW_SAMPLE_READY);
  }
}
...
Adafruit_VL53L0X::setMeasurementTimingBudgetMicroSeconds(uint32_t budget_us) {
  Status = VL53L0X_SetMeasurementTimingBudgetMicroSeconds(&MyDevice, budget_us);
  return (Status == VL53L0X_ERROR_NONE);
}
...
uint32_t Adafruit_VL53L0X::getMeasurementTimingBudgetMicroSeconds() {
  uint32_t budget_us;
  Status =
      VL53L0X_GetMeasurementTimingBudgetMicroSeconds(&MyDevice, &budget_us);
  return (budget_us);
}
...
Adafruit_VL53L0X::setVcselPulsePeriod(VL53L0X_VcselPeriod VcselPeriodType,
  ...
  Status =
      VL53L0X_SetVcselPulsePeriod(&MyDevice, VcselPeriodType, VCSELPulsePeriod);
  return (Status == VL53L0X_ERROR_NONE);
}
...
Adafruit_VL53L0X::getVcselPulsePeriod(VL53L0X_VcselPeriod VcselPeriodType) {
  uint8_t cur_period;
  Status = VL53L0X_GetVcselPulsePeriod(&MyDevice, VcselPeriodType, &cur_period);
  return (cur_period);
}
...
boolean Adafruit_VL53L0X::setLimitCheckEnable(uint16_t LimitCheckId,
  ...
  Status =
      VL53L0X_SetLimitCheckEnable(&MyDevice, LimitCheckId, LimitCheckEnable);
  return (Status == VL53L0X_ERROR_NONE);
}
...
uint8_t Adafruit_VL53L0X::getLimitCheckEnable(uint16_t LimitCheckId) {
  ...
  uint8_t cur_limit;
  Status = VL53L0X_GetLimitCheckEnable(&MyDevice, LimitCheckId, &cur_limit);
  return (cur_limit);
}
...
boolean Adafruit_VL53L0X::setLimitCheckValue(uint16_t LimitCheckId,
  ...
  Status = VL53L0X_SetLimitCheckValue(&MyDevice, LimitCheckId, LimitCheckValue);
  return (Status == VL53L0X_ERROR_NONE);
}
...
FixPoint1616_t Adafruit_VL53L0X::getLimitCheckValue(uint16_t LimitCheckId) {
  ...
  FixPoint1616_t LimitCheckValue;
  Status =
      VL53L0X_GetLimitCheckValue(&MyDevice, LimitCheckId, &LimitCheckValue);
