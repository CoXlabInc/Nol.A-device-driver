...
bool BQ27441::begin(void)
{
	...
	this->wire.begin(); // Initialize I2C master
...
{
	return readWord(COMMAND_VOLTAGE);
}
...
int16_t BQ27441::current(current_measure type)
{
	...
	{
	case AVG:
		current = (int16_t) readWord(COMMAND_AVG_CURRENT);
		break;
	case STBY:
		current = (int16_t) readWord(COMMAND_STDBY_CURRENT);
		break;
	case MAX:
		current = (int16_t) readWord(COMMAND_MAX_CURRENT);
		break;
	}
...
uint16_t BQ27441::capacity(capacity_measure type)
{
	...
	{
	case REMAIN:
		return readWord(COMMAND_REM_CAPACITY);
		break;
	case FULL:
		return readWord(COMMAND_FULL_CAPACITY);
		break;
	case AVAIL:
		capacity = readWord(COMMAND_NOM_CAPACITY);
		break;
	case AVAIL_FULL:
		capacity = readWord(COMMAND_AVAIL_CAPACITY);
		break;
	case REMAIN_F:
		capacity = readWord(COMMAND_REM_CAP_FIL);
		break;
	case REMAIN_UF:
		capacity = readWord(COMMAND_REM_CAP_UNFL);
		break;
	case FULL_F:
		capacity = readWord(COMMAND_FULL_CAP_FIL);
		break;
	case FULL_UF:
		capacity = readWord(COMMAND_FULL_CAP_UNFL);
		break;
	case DESIGN:
		capacity = readWord(EXTENDED_CAPACITY);
	}
...
{
	return (int16_t) readWord(COMMAND_AVG_POWER);
}
...
uint16_t BQ27441::soc(soc_measure type)
{
	...
	{
	case FILTERED:
		socRet = readWord(COMMAND_SOC);
		break;
	case UNFILTERED:
		socRet = readWord(COMMAND_SOC_UNFL);
		break;
	}
...
{
	uint16_t sohRaw = readWord(COMMAND_SOH);
...
uint16_t BQ27441::temperature(temp_measure type)
{
	...
	{
	case BATTERY:
		temp = readWord(COMMAND_TEMP);
		break;
	case INTERNAL_TEMP:
		temp = readWord(COMMAND_INT_TEMP);
		break;
	}
	return temp;
}
...
bool BQ27441::GPOUTPolarity(void)
{
	...
	return (opConfigRegister & OPCONFIG_GPIOPOL);
}
...
bool BQ27441::setGPOUTPolarity(bool activeHigh)
{
	...
	if ((activeHigh && (oldOpConfig & OPCONFIG_GPIOPOL)) ||
        (!activeHigh && !(oldOpConfig & OPCONFIG_GPIOPOL)))
		return true;
...
bool BQ27441::GPOUTFunction(void)
{
	...
	return (opConfigRegister & OPCONFIG_BATLOWEN);
}
...
bool BQ27441::setGPOUTFunction(gpout_function function)
{
	...
	// Check to see if we need to update opConfig:
	if ((function && (oldOpConfig & OPCONFIG_BATLOWEN)) ||
        (!function && !(oldOpConfig & OPCONFIG_BATLOWEN)))
		return true;
...
bool BQ27441::setSOCFThresholds(uint8_t set, uint8_t clear)
{
	...
	thresholds[0] = constrain(set, 0, 100);
	thresholds[1] = constrain(clear, 0, 100);
	return writeExtendedData(ID_DISCHARGE, 2, thresholds, 2);
}
...
{
	return readExtendedData(ID_STATE, 26);
}
...
{
	return executeControlWord(CONTROL_PULSE_SOC_INT);
}
...
{
	return readControlWord(CONTROL_DEVICE_TYPE);
}
...
bool BQ27441::exitConfig(bool resim)
{
	...
	if (resim)
	{
		...
		{
			int16_t timeout = BQ72441_I2C_TIMEOUT;
			while ((timeout--) && ((flags() & FLAG_CFGUPMODE)))
				delay(1);
			if (timeout > 0)
			{
...
{
	return readWord(COMMAND_FLAGS);
}

// Read the CONTROL_STATUS subcommand of control()
uint16_t BQ27441::status(void)
{
	return readControlWord(CONTROL_STATUS);
}
...
{
	return readWord(EXTENDED_OPCONFIG);
}
...
bool BQ27441::writeOpConfig(uint16_t value)
{
	...
	return writeExtendedData(ID_REGISTERS, 0, opConfigData, 2);
}
...
bool BQ27441::blockDataControl(void)
{
	...
	return i2cWriteBytes(EXTENDED_CONTROL, &enableByte, 1);
}
...
{
	uint8_t csum;
	i2cReadBytes(EXTENDED_CHECKSUM, &csum, 1);
	return csum;
}
...
{
	uint8_t ret;
	uint8_t address = offset + EXTENDED_BLOCKDATA;
...
{
	uint8_t data[32];
	i2cReadBytes(EXTENDED_BLOCKDATA, data, 32);
...
int16_t BQ27441::i2cReadBytes(uint8_t subAddress, uint8_t * dest, uint8_t count)
{
	...
	this->wire.write(subAddress);
	this->wire.endTransmission(true);

	this->wire.requestFrom(_deviceAddress, count);
	while ((this->wire.available() < count) && timeout--)
		delay(1);
	if (timeout)
	{
		for (int i=0; i<count; i++)
		{
			dest[i] = this->wire.read();
		}
	}
