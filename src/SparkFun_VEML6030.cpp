...
#include "SparkFun_VEML6030.hpp"

SparkFun_VEML6030::SparkFun_VEML6030(TwoWire &wirePort, uint8_t address)
  : _i2cPort(wirePort), _address(address) {
}

bool SparkFun_VEML6030::begin() {
...
void SparkFun_VEML6030::setGain(float gainVal){
  uint16_t bits; 
...
// sensors is behind dark glass, where as the lowest setting should be used in
// dark rooms. The datasheet suggests always leaving it at around 1/4 or 1/8.
float SparkFun_VEML6030::readGain(){
...
void SparkFun_VEML6030::setIntegTime(uint16_t time){ 
  uint16_t bits;
...
// sensor) of the ambient light sensor. Higher integration time leads to better
// resolution but slower sensor refresh times. 
uint16_t SparkFun_VEML6030::readIntegTime(){
...
void SparkFun_VEML6030::setProtect(uint8_t protVal){
  uint16_t bits; 
...
uint8_t SparkFun_VEML6030::readProtect(){
  uint16_t regVal = _readRegister(SETTING_REG); 
...
void SparkFun_VEML6030::shutDown(){
...
void SparkFun_VEML6030::powerOn(){
...
void SparkFun_VEML6030::enablePowSave(){
...
void SparkFun_VEML6030::disablePowSave(){
...
uint8_t SparkFun_VEML6030::readPowSavEnabled(){
...
void SparkFun_VEML6030::setPowSavMode(uint16_t modeVal){
  uint16_t bits; 
...
// increases power saving. The datasheet suggests enabling these modes when
// continually sampling the sensor. 
uint8_t SparkFun_VEML6030::readPowSavMode(){
  uint16_t regVal = _readRegister(POWER_SAVE_REG); 
...
uint8_t SparkFun_VEML6030::readInterrupt(){
...
void SparkFun_VEML6030::setIntLowThresh(uint32_t luxVal){
  if (luxVal > 120000)
    return;
...
uint32_t SparkFun_VEML6030::readLowThresh(){
...
// It takes a lux value as its paramater.
void SparkFun_VEML6030::setIntHighThresh(uint32_t luxVal){
  if (luxVal > 120000)
    return;
...
uint32_t SparkFun_VEML6030::readHighThresh(){
...
uint32_t SparkFun_VEML6030::readLight(){
...
// determined based on current gain and integration time settings. If the lux
// value exceeds 1000 then a compensation formula is applied to it. 
uint32_t SparkFun_VEML6030::readWhiteLight(){
...
// etc. etc. 
uint32_t SparkFun_VEML6030::_luxCompensation(uint32_t _luxVal){ 
...
uint32_t SparkFun_VEML6030::_calculateLux(uint16_t _lightBits){
  float _luxConv; 
  uint8_t _convPos;  
...
// that.  
uint16_t SparkFun_VEML6030::_calculateBits(uint32_t _luxVal){
  float _luxConv; 
  uint8_t _convPos;  
...
// position.
void SparkFun_VEML6030::_writeRegister(uint8_t _wReg, uint16_t _mask,
...
uint16_t SparkFun_VEML6030::_readRegister(uint8_t _reg) {
  uint16_t _regValue; 
